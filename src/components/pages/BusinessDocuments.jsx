      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-6">
        ...
        <div className="mb-4 lg:mb-6">
          ...
          >
            ...
            <span className="text-sm lg:text-base">Back to Documents</span>
          </button>
          <h1 className="text-xl lg:text-2xl font-semibold text-gray-900">Business Documents</h1>
          <p className="text-sm lg:text-base text-gray-600 mt-1">View and manage your directors/partners and organization documents</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
          {/* Directors/Partners Documents Card */}
          <div
            className="bg-white rounded-xl lg:rounded-2xl shadow-[0_2px_8px_rgba(0,-0,0,0.05)] border border-gray-100 p-4 sm:p-6 cursor-pointer hover:shadow-md transition-shadow group"
            onClick={() => navigate("/business-directors")}
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg lg:text-xl font-semibold text-gray-900">Directors/Partners</h2>
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">
                  {directorsPartnersDocuments.length} {directorsPartnersDocuments.length === 1 ? "doc" : "docs"}
                </span>
                <svg className="w-5 h-5 text-gray-400 group-hover:text-[#022B51] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </div>
            </div>
            <div className="text-center py-6 lg:py-8 bg-gray-50/50 rounded-xl border border-dashed border-gray-200 group-hover:border-purple-200 transition-colors">
              <svg
                className="w-12 h-12 lg:w-16 lg:h-16 text-purple-400 mx-auto mb-3 lg:mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-gray-400 text-[11px] lg:text-xs">Click to view documents by type</p>
            </div>
          </div>

          {/* Organizations Documents Card */}
          <div className="bg-white rounded-xl lg:rounded-2xl shadow-[0_2px_8px_rgba(0,-0,0,0.05)] border border-gray-100 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg lg:text-xl font-semibold text-gray-900">Organizations</h2>
              <span className="px-3 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                {organizationDocuments.length} {organizationDocuments.length === 1 ? "doc" : "docs"}
              </span>
            </div>
            {organizationDocuments.length > 0 ? (
              <div className="space-y-3 max-h-[400px] lg:max-h-[500px] overflow-y-auto pr-1">
                {organizationDocuments.map((doc, index) => (
                  <div
                    key={index}
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-3 lg:p-4 bg-gray-50/50 hover:bg-gray-50 rounded-xl border border-gray-100 transition-all gap-3 sm:gap-0"
                  >
                    <div className="flex items-center space-x-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm lg:text-base font-medium text-gray-900 truncate" title={doc.name}>{doc.name}</p>
                        {doc.service && doc.service !== "-" && (
                          <p className="text-[11px] lg:text-xs text-gray-500 truncate" title={doc.service}>Service: {doc.service}</p>
                        )}
                      </div>
                    </div>
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 text-xs lg:text-sm font-medium text-[#022B51] border border-[#022B51] rounded-lg hover:bg-[#015079] hover:text-white transition-colors flex-shrink-0 sm:ml-2"
                    >
                      View
                    </button>
                  </div>
              ...
              <div className="text-center py-6 lg:py-8 bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
                <svg
                ...
                <p className="text-gray-500 text-xs lg:text-sm mb-1">No organization documents found</p>
                <p className="text-gray-400 text-[11px] lg:text-xs">Documents will appear here after you submit registrations</p>
              </div>
            )}
          </div>
        </div>
