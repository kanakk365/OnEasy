...
function GSTRegistrationDetails() {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('packages');
  const [expandedSection, setExpandedSection] = useState(null);

  // Fetch packages from API
  const { packages, loading: packagesLoading } = usePackages('gst-registration');
  ...
  const prerequisites = [
    ...
    {
      title: 'GST Exempted Goods/Services',
      description: 'Entities dealing in GST-exempted goods/services are not required to register.'
    },
    {
      title: 'GST Turnover Limits',
      description: 'Businesses with turnover below ₹20 lakhs can voluntarily register for GST. For service providers exceeding ₹20 lakhs and suppliers of goods exceeding ₹40 lakhs, GST registration is mandatory. Special provisions apply for businesses operating in special category states.'
    }
  ];
        ...
        return (
          <PackagesSection
            ...
            onGetStarted={async (selectedPackage) => {
              try {
                console.log('Initiating payment for GST Registration:', selectedPackage.name);
                ...
                if (result.success) {
                  if (result.showPopup) {
                    ...
                    // GST Registration has a form, so this shouldn't happen, but handle it just in case
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-[#022B51] text-white rounded-lg hover:bg-[#015079] transition-colors font-medium shadow-sm"
          >
