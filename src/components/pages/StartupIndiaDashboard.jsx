...
import { getMyRegistrations } from '../../utils/startupIndiaApi';

function StartupIndiaDashboard() {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      console.log('📊 Startup India registrations response:', result);
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              View and manage your Startup India registration applications
            </p>
          </div>
          <button
        ...
        {registrations.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">
              You haven't submitted any Startup India registrations yet.
            </p>
            <button
              onClick={() => navigate('/startup-india-details')}
          ...
          <div className="space-y-4">
            {registrations.map((registration) => {
              ...
              return (
                <div
                  key={ticketId}
                  className="bg-white rounded-lg shadow-sm p-3 sm:p-6 hover:shadow-md transition-shadow"
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-lg font-semibold text-[#28303F]">
                          {registration.business_name || 'Startup India Registration'}
                        </h3>
                        <span
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                        <div>
                          ...
                          <span className="text-[#022B51]">{ticketId}</span>
                        </div>
                        <div>
                    ...
                    <div className="flex flex-col gap-2 ml-4">
                      <button
                        onClick={(e) => {
                          ...
                          navigate(`/startup-india-form?ticketId=${ticketId}`);
                        }}
                        className="px-4 py-2 text-sm text-white rounded-md hover:opacity-90 transition-all flex items-center gap-2" style={{ background: "linear-gradient(180deg, #022B51 0%, #015079 100%)" }}
                      >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/startup-india/view/${ticketId}`);
                        }}
                        className="px-4 py-2 text-sm border border-[#022B51] text-[#022B51] rounded-md hover:bg-[#015079] hover:text-white transition-colors"
                      >
                        View Details
                      </button>
                    </div>
...
export default StartupIndiaDashboard;
