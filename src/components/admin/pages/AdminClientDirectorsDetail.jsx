...
function AdminClientDirectorsDetail() {
  ...
  const { userId, orgId, documentType } = useParams();
  ...
  const loadClientAndDocuments = async () => {
    try {
      ...
      const response = await apiClient.get(`/users-page/directors-partners-documents/${userId}?organizationId=${orgId}`).catch(() => ({ 
        success: false, 
        data: {}
      }));
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      const folder = `user-profiles/${userId}/organizations/${orgId}/directors-partners`;
      ...
      const response = await apiClient.post(`/users-page/upload-directors-partners-document/${userId}`, {
  ...
  const handleDeleteDocument = async (docId) => {
    try {
      ...
      const response = await apiClient.delete(`/users-page/directors-partners-documents/${docId}?organizationId=${orgId}`);
  ...
  // Render icon component (same as BusinessDirectorsDocumentDetail)
...
export default AdminClientDirectorsDetail;
